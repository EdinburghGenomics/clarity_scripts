...
def parseSampleSheet():
    ...
    for i in range(0, wb_sheet.nrows):
        ...
        for idx, cell_obj in rowval:
            ...
            if objtype == 'text' and cell_obj.value == 'Form Title:':
                raise ValueError("Cover page present in manifest file. Please delete cover page from manifest file.")
    ...
    for i in sorted(rawdata.keys()):
        ...
        if type(dataline[2]) is float:
            dataline[2] = str(int(dataline[
                                      2]))  # DF Excel sheet parses value as float if number only so convert to int to remove decimal place .0 and then convert to str so .strip() doesn't error
        if type(dataline[1]) is float:
            dataline[1] = str(int(dataline[1]))
        # print dataline[0]
        namelist[dataline[2].strip('')] = dataline
        namecountlist.append(dataline[2])

    for value in namecountlist:
        if namecountlist.count(value) > 1:
            raise ValueError("2D Barcode duplicated in Excel file: " + value)

    # now batch get the samples by barcode
    bArr = []
    for name in namelist.keys():
        bArr.append('udf.' + urllib.quote('2D Barcode') + '=' + urllib.quote(bcode))
    ...
    if re.search('<message>Links must be provided when using batch retrieval</message>', mXML):
        raise ValueError("No barcodes in LIMS matching manifest")
    # print("mXML"+mXML)
    ...
    for sDOM in mDOM.getElementsByTagName('smp:sample'):
        ...
        if sName not in SAMPLE_DATA.keys():
            SAMPLE_DATA[sName] = []
