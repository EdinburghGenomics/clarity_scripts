...
class GenerateHamiltonInputUPL(StepEPP):
    ...
    def _run(self):
        ...
        with open(self.shared_drive_path +'MAKE_QPCR-1_TO_23_INPUT.csv','w') as csvFile:
            writer = csv.writer(csvFile)
            writer.writerows(csv_array)
...
def main():
    ...
    p.add_argument('-d', '--shared_drive_path', type=str, required=True, help='Shared drive path location for Hamilton input file')
    args = p.parse_args()
