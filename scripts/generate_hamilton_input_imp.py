...
class GenerateHamiltonInputIMP(GenerateHamiltonInputEPP):
    ...
    output_file_name = 'KAPA_TRANSFER_CFP_TO_IMP.csv'
            ...
            if input_art.type == 'Analyte':
                ...
                if len(outputs) > 1:
                    raise InvalidStepError(
                        message='Multiple outputs found for an input %s. This step is not compatible with replicates.' % (
                            input_art.name))

                # only 1 output expected so output is always position 0 in the outputs list
                output = outputs[0]
                ...
                csv_line = [input_art.container.name, input_location, output.container.name, output_location,
                            cfp_volume]
...
if __name__ == '__main__':
    sys.exit(GenerateHamiltonInputIMP().run())
