...
class AssignNextStepUPL(StepEPP):
    """
    This script assigns the next step for samples in the User Prepared Library Plate Receipt step. It assigns the next
    step as either the next step or review depending on whether the plate condition step UDFs are answered as Yes or No
    by the lab team.
    """
    ...
    def _run(self):
        ...
        # obtain the next actions in the step then create a list of dicts for next_actions for the step for all
        # artifacts
        next_actions = actions.next_actions

        # check step UDFs answered by lab team to confirm that sample is in good condition. If UDFs both answered "Yes",
        # then set next action as the next step, if one or more UDF is set to "No" then set as "review". The next step
        # in the protocol is found rather than hard coded.
