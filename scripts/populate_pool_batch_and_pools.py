...
class PopulatePoolBatchPools(StepEPP):
    ...
    def create_pool_batch_id(self):
        ...
        raise ValueError('Cannot allocate more than %s pool batch IDs date %s ' % (today))
    ...
    def _run(self):
        ...
        self.process.analytes()[0].container.name = self.create_pool_batch_id()
        ...
        adapter_type = self.lims.get_reagent_types(name=self.process.all_inputs()[0].reagent_labels[0])[0]

        # Name all pools and assign the prep workflow and adapter type to them
        for o_art in self.process.analytes():
            o_art.name = self.create_pool_id(o_art)
