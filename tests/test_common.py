...
class FakeEntitiesMaker:
    ...
    def create_a_fake_steppools(self, output_artifacts, input_artifacts, pool_size, **kwargs):
        steppools = self.create_instance(StepPools)
        pool_count = 0
        input_count = 0
        number_of_inputs = len(input_artifacts)
        number_of_pools = len(output_artifacts)

        while pool_count < number_of_pools:
            input_list_count = 0
            input_tuple = ()
            while input_list_count < pool_size and input_count < number_of_inputs:
                input_tuple = input_tuple + (input_artifacts[input_count],)
                input_count += 1
                input_list_count += 1
    ...
    def create_a_fake_step(self, step_id=None, selected_containers=None, step_udfs=None, output_artifacts=None,
                           input_artifacts=None, next_action=None, pool_size=None, **kwargs):
    ...
    def create_a_fake_process(self, nb_input=1, output_per_input=1, input_name=None, output_name=None,
                              ...
                              output_container_population_patterns=None, pool_size=None, **kwargs):
        ...
        :param pool_size: Define the number of inputs per output pool. Defining this parameter causes StepPools
        to be created. The total number of outputs will be the number of pools required to contain nb_inputs.
        """
        # Create the projects
        ...
        #if there are no pools present in the process then create the outputs and build in the input_output_maps
        # using the inputs as the key as there are 1 or more replicate outputs per input
        if pool_size == None:
            ...
            for a in inputs:
                ...
                for n in range(output_per_input):
                    ...
                    outputs.append(oa)
                    output_map = {'uri': oa, 'limsid': oa.id, 'output-generation-type': 'PerInput',
                                  'output-type': o_type}
                    input_output_maps.append((input_map, output_map))

        else:
            # if there are pools present in the process then create the outputs independent from the inputs and build
            # the input_output_maps using the StepPool structure
            input_map_dict = {}
            output_map_dict = {}
            for a in inputs:
                input_map_dict[a] = {'uri': a, 'limsid': a.id, 'post-process-uri': a}

            number_of_outputs = -(-nb_input // pool_size)  # divides and rounds up
            output_count = 1
            while output_count <= number_of_outputs:
                o_type = _resolve_next(output_type)
                ...
                oa = self.create_a_fake_artifact(is_output_artifact=True, artifact_name=_resolve_next(output_name),
                                                 ...
                                                 reagent_label=output_reagent_label,
                                                 sample=a.samples[0], container=ocontainer,
                                                 artifact_position=output_position,
                                                 **kwargs)
                used_output_containers.add(oa.container)
                outputs.append(oa)
                output_map_dict[oa.name] = {'uri': oa, 'limsid': oa.id, 'output-generation-type': 'PerInput',
                                            'output-type': o_type}
                output_count += 1
        ...
        step = self.create_a_fake_step(step_id=p.id, selected_containers=list(used_output_containers),
                                       output_artifacts=outputs, input_artifacts=inputs,
                                       pool_size=pool_size, **kwargs)
        # create input_output_map based on the StepPool if pool_size populated
        if not pool_size == None:
            for pool in step.pools.pooled_inputs:
                for input in step.pools.pooled_inputs[pool][1]:
                    input_output_maps.append((input_map_dict[input], output_map_dict[pool]))
