...
class TestAssignmentNextStepSpectramax(TestEPP):
    ...
    def setUp(self):
        ...
        self.actions = Mock(
            next_actions=[{'artifact': Mock(id='ai1')},
                          {'artifact': Mock(id='ai2', samples=[Mock(udf={'Prep Workflow': 'KAPA DNA Library Prep'})])},
                          {'artifact': Mock(id='ai3')},
                          {'artifact': Mock(id='ai4', samples=[Mock(udf={'PreSeqLab Fragment Analyser Complete': True})])}
                          ]
        )
        ...
        protocol=Mock(steps=[self.protostep,Mock(),Mock(),Mock()])
        self.patched_protocol = patch('scripts.next_step_assignment_spectramax.Protocol', return_value=protocol)
