...
from tests.test_common import TestEPP, NamedMock, PropertyMock, FakeEntitiesMaker


class TestParseManifest(TestEPP):
    sample1 = NamedMock(real_name='Key1')
    sample2 = NamedMock(real_name='Key2')
    container = NamedMock(real_name='container1', type=NamedMock(real_name='96 well plate'))
    location = [container, 'A:1']
    artifact1 = NamedMock(real_name='Key1', samples=[sample1], container=container, location=location)
    artifact2 = NamedMock(real_name='Key2', samples=[sample2], container=container, location=location)
    ...
    def setUp(self):
        self.epp = parse_manifest.ParseManifest(
            self.default_argv + ['--manifest', join(dirname(abspath(__file__)), 'assets', 'Manifest_To_Parse.xlsx')])
        self.epp2 = parse_manifest.ParseManifest(self.default_argv + ['--manifest',
                                                                      join(dirname(abspath(__file__)), 'assets',
                                                                           'Manifest_To_Parse_too_many.xlsx')])
    ...
    def test_parse_manifest_happy_path(self):
        ...
        self.epp.process = fem.create_a_fake_process(nb_input=2, sample_name=iter(['Key1', 'Key2']), step_udfs=self.process_udfs)
    ...
    def test_parse_manifest_sample_too_few(self):
        with self.patch_process3, self.patched_lims:
            ...
            assert str(
                e.value) == 'The number of samples in the step does not match the number of samples in the manifest'
